...
import tv.pouyaam.bottrader.databinding.ActivityMainBinding
import tv.pouyaam.bottrader.domain.model.cryptocompare.historical.daily.DataItemDomain
import tv.pouyaam.bottrader.ui.base.ActivityBase
...
//            btc_price.text = "${it}"
    ...
    private fun rsiStrats(data: List<DataItemDomain>) {
        ...
        val bars = data.map {
            ...
            val beginTime = ZonedDateTime.ofInstant(Instant.ofEpochMilli(it.time.toLong() * 1000), ZoneId.systemDefault())
        ...
        // Analysis
        Log.d("DEBUG_MAIN",
            "Total profit for the strategy: " + TotalProfitCriterion().calculate(series, tradingRecord)
        )

        btc_price.text = "Number of trades for the strategy: ${tradingRecord.tradeCount}\n total profit for the strategy: ${TotalProfitCriterion().calculate(series, tradingRecord)}"
    }
